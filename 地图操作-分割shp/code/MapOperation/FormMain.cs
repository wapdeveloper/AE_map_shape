...
namespace MapOperation
{
    public partial class FormMain : Form
    {
        ...
        private void btnAddShapefile_Click(object sender, EventArgs e)
        {
            ...
            //pGeometryDefEdit.SpatialReference_2 = mainMapControl.SpatialReference;
            ...
            //if (i == -1)
            //    pFeatureClass2 = pFeatureWorkspace.CreateFeatureClass(fileName + ".shp", pFields, null, null, esriFeatureType.esriFTSimple, "Shape", "");
            //else
            //    pFeatureClass2 = pFeatureWorkspace.CreateFeatureClass(fileName, pFields, null, null, esriFeatureType.esriFTSimple, "Shape", "");
            ...
            //pFeature.set_Value(pFeature.Fields.FindField("editorName"), "a");
            //pFeature.set_Value(pFeature.Fields.FindField("editorName"), "b");
            //pFeature.Store();


            //mainMapControl.AddShapeFile(filePath, fileName + ".shp");
            ...
            try
            {
                ...
                {
                    frmAttribute = new FormAtrribute(mainMapControl);
                }
                frmAttribute.CurFeatureLayer = pFeatureLayer;
                frmAttribute.InitUI();

                Grid grid = frmAttribute.gd;

                //多少条记录
                int records = grid.Rows.Count;
                //复制多少个shp文件
                ...
                {
                    string shp = pFilePath + "\\" + grid.Rows[i].columns[3]+ "." + "shp";
                    File.Copy(pFullPath, shp);

                    string shx_Path = pFilePath + "\\" + spname[0] + "." + "shx";
                    string shx = pFilePath + "\\" + grid.Rows[i].columns[3] + "." + "shx";
                    File.Copy(shx_Path, shx);

                    string sbx_Path = pFilePath + "\\" + spname[0] + "." + "sbx";
                    string sbx = pFilePath + "\\" + grid.Rows[i].columns[3] + "." + "sbx";
                    File.Copy(sbx_Path, sbx);

                    string sbn_Path = pFilePath + "\\" + spname[0] + "." + "sbn";
                    string sbn = pFilePath + "\\" + grid.Rows[i].columns[3] + "." + "sbn";
                    File.Copy(sbn_Path, sbn);

                    string prj_Path = pFilePath + "\\" + spname[0] + "." + "prj";
                    string prj = pFilePath + "\\" + grid.Rows[i].columns[3] + "." + "prj";
                    File.Copy(prj_Path, prj);

                    string dbf_Path = pFilePath + "\\" + spname[0] + "." + "dbf";
                    string dbf = pFilePath + "\\" + grid.Rows[i].columns[3] + "." + "dbf";
                    File.Copy(dbf_Path, dbf);


                    //string shx_Path = pFilePath + "\\" + spname[0] + "." + "shx";
                    //string shx = pFilePath + "\\" + spname[0] + "_" + (i + 1) + "." + "shx";
                    //File.Copy(shx_Path, shx);

                    //string sbx_Path = pFilePath + "\\" + spname[0] + "." + "sbx";
                    //string sbx = pFilePath + "\\" + spname[0] + "_" + (i + 1) + "." + "sbx";
                    //File.Copy(sbx_Path, sbx);

                    //string sbn_Path = pFilePath + "\\" + spname[0] + "." + "sbn";
                    //string sbn = pFilePath + "\\" + spname[0] + "_" + (i + 1) + "." + "sbn";
                    //File.Copy(sbn_Path, sbn);

                    //string prj_Path = pFilePath + "\\" + spname[0] + "." + "prj";
                    //string prj = pFilePath + "\\" + spname[0] + "_" + (i + 1) + "." + "prj";
                    //File.Copy(prj_Path, prj);

                    //string dbf_Path = pFilePath + "\\" + spname[0] + "." + "dbf";
                    //string dbf = pFilePath + "\\" + spname[0] + "_" + (i + 1) + "." + "dbf";
                    //File.Copy(dbf_Path, dbf);
                    ...
                    int pIndex1 = shp.LastIndexOf("\\");
                    string pFilePath1 = shp.Substring(0, pIndex1); //文件路径
                    string pFileName1 = shp.Substring(pIndex1 + 1); //文件名
                    //实例化ShapefileWorkspaceFactory工作空间，打开Shape文件
                    pWorkspaceFactory = new ShapefileWorkspaceFactory();
                    pFeatureWorkspace = (IFeatureWorkspace)pWorkspaceFactory.OpenFromFile(pFilePath1, 0);
                    //创建并实例化要素集
                    IFeatureClass pFeatureClass1 = pFeatureWorkspace.OpenFeatureClass(pFileName1);
                    IFeatureLayer pFeatureLayer1 = new FeatureLayer();
                    pFeatureLayer1.FeatureClass = pFeatureClass1;
			        ...
			        {
                        if (j < i)
                            DeletebyITable(pFeatureClass1, 0);
                        else
                            DeletebyITable(pFeatureClass1, 1);
			        }

                    mainMapControl.Map.AddLayer(pFeatureLayer1);
                    mainMapControl.ActiveView.Refresh();
                    //同步鹰眼
                    SynchronizeEagleEye();
                ...
                //for (int j = 0; j < records - 1; j++)
                //{
                //    DeletebyITable(pFeatureClass, 1);
                //}
        ...
        private void DeletebyITable(IFeatureClass PFeatureclass,int num)
        {
            IQueryFilter pQueryFilter = new QueryFilterClass();
            pQueryFilter.WhereClause = "FID =" + num;
