...
namespace MapOperation
{
    public partial class FormAtrribute : Form
    {
        ...
        public FormAtrribute(ESRI.ArcGIS.Controls.AxMapControl mainMapControl)
        {
        ...
        public void InitUI()
        {
            ...
            gd.Rows = new List<Columns>();
            int w = 0;
            while (pFeature != null)
            {
                //colist[w].Rows = new List<object>();
                pDataRow = pFeatDT.NewRow();

                Columns cols = new Columns();
                cols.columns = new List<object>();
                //获取字段属性
                for (int k = 0; k < pFeatDT.Columns.Count; k++)
                {
                    pDataRow[k] = pFeature.get_Value(k);
                    cols.columns.Add(pDataRow[k]);
                    //colist[w].Rows.Add(pDataRow[k]);
                }
                gd.Rows.Add(cols);
                w++;
